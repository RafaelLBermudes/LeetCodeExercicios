...
int **fourSum( int *nums, int numsSize, int target, int *returnSize, int **returnColumnSizes ) {
	...
	for ( int i = 0; i < numsSize - 3; i++ ) {
		...
		for ( int j = i + 1; j < numsSize - 2; j++ ) {
			...
			int z = numsSize - 1;
			while ( x < z ) {
				long long int check = nums[i] + nums[j] + nums[x] + nums[z];
				if ( check > target )
					z--;
					...
					if ( linhas > capacidade ) {
						capacidade *= 2;
						res = realloc( res, sizeof(int *) * capacidade );
						// returnColumnSizes = (int *)realloc( returnColumnSizes, sizeof(int) * capacidade );
					}
					...
					while ( x > j + 1 && nums[x] == nums[x + 1] )
						x++;

					while ( z < nums - 1 && nums[z] == nums[z - 1] )
						z--;
